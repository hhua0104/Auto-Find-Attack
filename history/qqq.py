...
def print_attack(runtime):
    ...
    while int(compare_time)>int(runtime):
        ...
        for j in range(0,len(all_data_tmp)):
            ...
            if str(all_data_tmp[j]["data"]).find(tmp)!=-1:
                ...
                all_data_tmp.append({'time':now_time,'data': message, 'num': 1, 'reason': reason})
    ...
    for j in range(0,len(all_data_tmp)):
        print '发现攻击'
        message1= '时间：' +all_data_tmp[j]["time"]+all_data_tmp[j]["data"]+' 攻击方式：' +all_data_tmp[j]["reason"]+' 攻击次数：' +str(all_data_tmp[j]["num"])
        print message1
